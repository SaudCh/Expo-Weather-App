            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <TouchableOpacity
                        ...
                        style={{ margin: 10, borderColor: '#fff', padding: 10, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                        <View style={{ ...styles.card }}>
                            <View style={{ ...styles.hourCont }}>
                                <Text style={{ display: 'none' }}>{str = item.list[0].dt_txt}</Text>
                                <Text style={{ ...styles.text }}>{str.substring(11, 16)}</Text>
                                <Image
                                    style={{ height: 70, width: 70 }}
                                    source={{ uri: `https://openweathermap.org/img/wn/${item.list[0].weather[0].icon}@4x.png` }}
                                />
                                <Text style={{ ...styles.text }}>{Math.ceil(item.list[0].main.temp)}&deg;</Text>
                            </View>
                        </View>
                        <Button>
                            <Text>Hello</Text>
